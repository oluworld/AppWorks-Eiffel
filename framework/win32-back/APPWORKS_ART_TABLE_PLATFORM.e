...
feature -- win32 specific creation 

	get_ex_style : BIT 32 is
		do
--			Result := WS_EX_CLIENTEDGE 
		end -- get_ex_style
	get_style : BIT 32 is
		do
			Result := WS_CHILD or WS_VISIBLE or WS_BORDER or 
...
feature {NONE} -- private functions
	...
	WC_LISTVIEW_POINTER: POINTER is
		external "C macro inline use <commctrl.h>"
		alias "WC_LISTVIEW"
		end -- WC_LISTVIEW

	WC_LISTVIEW: STRING is 
		once
			!!Result.from_external (WC_LISTVIEW_POINTER)
		end -- WC_LISTVIEW

	init_list_view is
			-- Ensure that the common control DLL is loaded.
		once
			c_inline_c ("INITCOMMONCONTROLSEX icex;%N%N")
			
			c_inline_c ("icex.dwSize = sizeof(INITCOMMONCONTROLSEX);%N")
			c_inline_c ("icex.dwICC  = ICC_LISTVIEW_CLASSES;%N")
			c_inline_c ("InitCommonControlsEx(&icex);%N")
